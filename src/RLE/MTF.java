...
public class MTF extends Compression{
	// la table mesure la distance par rapport au caract�re actuel
	...
	public void compresser(String fin, boolean v){
		...
		try{
			...
			while((intLu=fluxLecture.read())!=-1){
				...
				// on r�cup�re l'index de l'octet dans la table
				int index=tableDistance.indexOf(byteLu);
				// on met � jour le tableau des index
	...
	public void decompresser(String fin, boolean v){
		...
		try{
			...
			// on fait l'exact oppos� de la compression
			
			if(v)System.out.print(">> D�compression...        ");
			...
			if(v)System.out.println("OK");
			tps+=System.currentTimeMillis();
			System.out.println("Fin de la d�compression en "+tps+" ms");
		}
		catch(IOException e){
			System.out.println(e);
		}
