...
public class HuffmanComp8 extends Compression{
	...
	public void compresser(String fin, boolean v){
		...
		try{
			...
			StringBuffer chaneCompresse=new StringBuffer();
			int buff;
			while((buff=fluxLecture.read())!=-1){
				chaneCompresse.append(tableCodage.get((byte)buff));
			}
			fluxLecture.close();
			if(v)System.out.println("OK");
			...
			byte[] liste=new byte[chaneCompresse.length()/(TAILLE_ECR)+1];
			int nombreBitsEcrits=0;
			int nombreEcritures=0;
			
			// on coupe la cha�ne en octet :
			while(chaneCompresse.length()>nombreBitsEcrits+TAILLE_ECR){
				liste[nombreEcritures]=string2Byte(chaneCompresse.substring(nombreBitsEcrits, nombreBitsEcrits+TAILLE_ECR));
				nombreBitsEcrits+=TAILLE_ECR;
				nombreEcritures++;
			}
			if(nombreEcritures<liste.length && nombreBitsEcrits<chaneCompresse.length())
				liste[nombreEcritures]=string2Byte(chaneCompresse.substring(nombreBitsEcrits));
			
			// on note le nombre de bit porteurs dans le dernier octet :
			int tailleDernierEntier=TAILLE_ECR-chaneCompresse.length()%TAILLE_ECR;
	...
	public void decompresser(String fin, boolean v){
		...
		try{
			...
			for(byte i:hc8.d){
				...
				ChaneCode.append(code8b);
			}
			
			// comme java utilise les deux conventions de signes, il faut diff�rencier pour g�rer les derniers bits non porteurs
			if(ChaneCode.charAt(ChaneCode.length()-TAILLE_ECR)=='1')
				ChaneCode.delete(ChaneCode.length()-TAILLE_ECR+1,ChaneCode.length()-TAILLE_ECR+hc8.td+1);
			else
				ChaneCode.delete(ChaneCode.length()-TAILLE_ECR,ChaneCode.length()-TAILLE_ECR+hc8.td);
			if(v)System.out.println("OK");
			if(v)System.out.print(">> Ecriture...             ");
			...
			while(!aTermin){
				...
				try{
					...
					fluxEcriture.writeByte(tableDecodage.get(ChaneCode.substring(t, t+j)));
				}
...
class HuffmanCompEcrit8 implements Serializable{
	...
	double t;//taille
	HuffmanCompEcrit8(int[] chaneBinaire,byte[] tailleChane,byte[] valeurChane, byte[] donnes, double taille,int tdi){
		decI=chaneBinaire;
		decT=tailleChane;
		decS=valeurChane;
		d=donnes;
